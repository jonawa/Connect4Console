...
public class NormalKI2 implements IPlayer{
	...
	public int turn() {
		...
		if(zz>=epsilon){
			...
			//Füge verbotene Züge zu veryBadIdeas hinzu.
			 ...
			 if (veryBadIdeas.size()==Game.COLUMNS){
				 ...
				 if (debug){
						System.out.println("Das ist alles nicht gut. Suche eine Spalte, die nicht voll ist.");
					}
				 return column;
			 }
			 else{
		...
		return possibleActions[zufallszahl];
