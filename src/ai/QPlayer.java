...
public class QPlayer implements IPlayer {
	...
	public int turn() {
		...
		//alpha legt fest, wie viel vom Ursprüglichen wert behalten werden soll 
		//und wie stark der wert mit jedem update angepasst werden soll
		double alpha = 0.05; // oder 0.1
		
		// newValue wird nicht mehr beliebig groß 
		...
		/*
		 * Speichere den aktuellen State und die ausgewählte Action. Falls die
		 * KI verliert, wird sie dazu aufgefordert auf das Verlieren zu
		 * reagieren (reactToWinrOrLose). Die KI muss also wissen, was sie
		 * gemacht hat um Q zu aktualisieren und sich zu bestrafen.
		 */
		lastState = currentState; // tiefe Kopie erstellen, wurde oben bereits
									// gemacht
		lastAction = action;

		// sichert db.txt im Projektordner, zum Testen aktuell, was in der DB
		// steht.
	...
	/**
	 * Nachdem die beste Aktion vom QPlayer ausgewählt wurde, wird diese Methode
	 * aufgerufen. Es werden die nächsten möglichen States für den QPlayer
	 * betrachet, und von dieses States die die Action gesucht mit dem höchsten
	 * Value. Dieser Value wird zurückgegeben. Da der QPlayer nicht direkt
	 * wieder dran ist, müssen auch alle unterschiedlichen Möglichkeiten für den
	 * Gegner durchgegangen werrden.
	 * 
	 * @param state
	 *            Spielfeld bevor der QPlayer seinen Stein geworfen hat.
	 * @param action
	 *            Zug der vom QPlayer ausgewählt worden ist.
	 * @return erfolgsversprechenster Wert, den der QPlayer aus diese State mit
	 *         dieser Action erreichen kann
	 */
	private int maxValueForNextStateAllActions(int[][] state, int action) {

		int maxOfallPossibleActions = Integer.MIN_VALUE;

		// Simulieren, QPlayer wirft seinen Stein, richtiges Board wird erst
		// verändert nach dem Zug
		...
		for (int actionOpponent : allActionOpponent) {
			...
			// Wähle höchsten Value aller möglichen nächsten Züge für QPlayer
			// aus.
	...
	/**
	 * Funktioniert ähnlich wie die Methode maxValueForNextStateAllActions,
	 * allerdings wird hier anstatt das Maximum der Durchschnitt aller Aktionen
	 * gebildet
	 * 
	 * @param state
	 * @param action
	...
	private int avgValueForNextStateAllActions(int[][] state, int action) {
		...
		if (count == 0)
			...
			return avgOfallPossibleActions / count;

	}

	/**
	 * Sieht sich das aktuelle Spielfeld(State) an und gibt alle möglichen
	 * Züge(Actions) zurück.
	 *
	 * Falls in der Datenbank Q bereits der State vorhanden ist, frage die
	 * möglichen Actions ab und gib sie zurück.
	 * 
	 * Falls in Q noch nicht der State vorhanden ist, teste welche Züge
	 * (Actions) möglich sind und schreibe sie anschließende in die DB (mit
	 * Value 0):
	 * 
	 * @param state
	 * @param forQPlayer
	 *            Wenn für QPlayer Action generiet werden, dann werden diese
	 *            auch in Q gespeichert, ansonsten nicht
	...
	private int[] generateActions(final int[][] state, boolean forQPlayer) {
		...
		// Value 0):
			...
			int[] allActions = new int[state[0].length]; // TODO Anzahl der
															// Zeilen Länge
															// oder? Es geht
															// aber um die
															// Spalten, hier
			...
			// in Datenbank Q schreiben und die Values mit 0 initialisieren, da
	...
	// Funktioniert nicht das checkWin nur das Board in Game prüft und nicht
	// einen beliebigen State
	...
	private int chooseBestAction(final int[][] currentState, final int[] actions) {
		...
		// TODO hier könnte man außerdem mit der isEndState Methode überprüfen,
		// ob man direkt gewinnen kann
		// Starte mit einer beliebigen Action, hier wird immer die erstmögliche
		// ausgewählt.
	...
	/**
	 * Bekommt einen State und betrachtet auf diesem State alle möglichen
	 * Actions und die Bewertung, die sich aus dieser Action ergeben. Gibt dann
	 * den maximalen zurück
	 * 
	 * @param state
	 * @return
	 */
	private int maxValueForState(final int[][] state) {
		// max[Q(next state, all actions] //wobei next state schon der ist, der
		// übergeben wird
		int maxValue;
		...
		// Wenn der State noch nicht vorhanden ist, kann an Q auch nicht
		// verändert werden, also betrachte Standardwert
	...
	/**
	 * Berechnet bei einem gegebenen State, den Durchschnitt aller Values, die
	...
	/**
	 * Da nicht alle Möglichkeiten zu gewinnen und zu verlieren in einer Matrix
	 * bewertet werden. Gibt diese Methode den Reward zurück mit Hilfe der
	 * checkWin Methode Also return 100, wenn gewonnen.
	 * 
	 * @param state
	 * @return
	 */
	// wird erstmal nicht gebraucht, da Reward erst nach dem Spielende über die
	// Methode reactToWinOrLose verteilt wird.
	...
	/**
	 * Sobald der Algo an einem Punkt angelangt ist, an dem er die nächsten
	 * möglichen Züge noch nicht in seiner Datenbank hat (Am Anfang ist das
	 * immer der Fall) wird diese Methode ausgeführt.
	 * 
	 * Die Methode generiert alle möglichen Züge (actions) basierend auf dem
	 * aktuellen Spielfeld (state) und fügt für diesen State dann alle möglichen
	...
	public void reactToWinOrLose(boolean win) {
		// teste ob der letzte State nicht der aktuelle State ist und wirklich
		// eine tiefe Kopie erstellet worden ist:
		...
		// wenn gewonnen, muss nichts passieren, KI hat sich schon selbst
		// belohnt, als der Gewinnzug ausgeführt wurde( mit REWARD)
		if (win) {
	...
	private void newTestCase() {
		...
		int[][] state = { { 1, 0, 0, 0, 0 }, { 1, 1, 0, 1, 0 }, { 2, 2, 2, 1, 0 }, { 1, 2, 1, 2, 0 } };
	...
	private void TestDB() {
		int[][] state = { { 0, 1, 0, 0 }, { 0, 1, 0, 1 }, { 0, 2, 2, 1 }, { 1, 2, 1, 2 } };
		...
		int[][] nextState = { { 0, 1, 0, 0 }, { 1, 1, 0, 1 }, { 2, 2, 2, 1 }, { 1, 2, 1, 2 } };
		Q.put(nextState, 0, -100);
		Q.put(nextState, 2, 50);
		Q.put(nextState, 3, 100);
		Q.saveDBToTxt();

		int[][] samestate = { { 0, 1, 0, 0 }, { 0, 1, 0, 1 }, { 0, 2, 2, 1 }, { 1, 2, 1, 2 } };
	...
	private void TestGenerateActions() {
		...
		int[][] board = { { 0, 1, 0, 0 }, { 0, 1, 0, 1 }, { 2, 2, 2, 1 }, { 1, 2, 1, 2 } };
		System.out.println(Helper.convertIntBoardToString(board));
		int[] actions = generateActions(board, true);
		int[] testActions = { 0, 2, 3 };

		System.out
				.println("Diese Actions sollten zurückgegeben werden: " + Helper.convertIntArrayToString(testActions));
		...
		int[][] board2 = { { 2, 1, 0, 0, 1, 2, 0 }, { 2, 1, 0, 0, 1, 2, 1 }, { 2, 1, 0, 0, 1, 2, 1 },
				{ 2, 1, 0, 0, 1, 2, 1 }, { 1, 2, 1, 2, 1, 2, 1 } };
		System.out.println(Helper.convertIntBoardToString(board2));
		int[] actions2 = generateActions(board2, true);
		int[] testActions2 = { 2, 3, 6 };

		System.out
				.println("Diese Actions sollten zurückgegeben werden: " + Helper.convertIntArrayToString(testActions2));
